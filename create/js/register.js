var Timeline = window.Timeline = $.Timeline = Timeline || {}
Timeline.create = {}
Timeline.create.data = {
	// Edit this with your recaptcha v3 site key
	reCAPTCHA_site_key : "6Lds0qsUAAAAAMnO1UTa6lGgI1aVcaGP-rfVX_3g",
	color: 1,

	base_url : 'http://create.localhost',
	inputs: ['edit-name', 'edit-pass', 'edit-email', 'edit-nick'],
	labelStyle : {
		'edit-name': 'left: -165px;',
		'edit-pass': 'left: -170px;',
		'edit-email': 'left: -120px;',
		'edit-nick': 'left: -150px;'
	},
	inputTips : {
		'edit-name': '<p>Unable to think of a username? Try using the one same as your nickname!</p>',
		...
		'edit-nick': "<p>Pick yourself a trendy nickname to show yourself off! <br> Make sure it's within 4 to 12 characters long.</p>"
	}
}

$.formElements = Timeline.create.elements = function () {
	$.formElements = $.Timeline.create.elements = {
...
$.Timeline.create.cookie = {};
$.Timeline.create.cookie.read = function (name) {
    var nameEQ = name + "=";
...
Timeline.create.ajax = function(request, data) {
	request = {
		url: $.Timeline.create.data.base_url + '/ajax/?action=' + encodeURIComponent(request),
		type: 'post',
		data: data,
		error: function(e){
			...
			$.Timeline.create.error("Unable to contact register server. Contact CPPS support.");
		},
		success: function(r) {
			if (!!!r) {
...
Timeline.create.error = function (txt, errors) {
...
Timeline.create.inputErrorMsg = function (error) {
	tags = [];
	for(var e in error) {
...
Timeline.create.validateResponse = function (r) {
	if (!!r.response.error) {
		tags = []
		tags2 = [];
	...
	$.Timeline.create.invalidate();
}

Timeline.create.invalidate = function() {
	inputs = ['input_nick', 'input_name', 'input_pass', 'input_email'];
...
Timeline.create.setup = function() {
	for(i = 0; i < $.Timeline.create.data.inputs.length; i++) {
		var inp = $.Timeline.create.data.inputs[i];

		$("#"+inp).off('focusin').on('focusin', function() {
			$("label[for="+this.id+"]").attr('style', $.Timeline.create.data.labelStyle[this.id]);
			$.formElements.tipTxt.html($.Timeline.create.data.inputTips[this.id]);
			$.formElements.tipBox.fadeIn();
			$(this).removeClass("valid").removeClass('error');

			//$.Timeline.create['click-' + this.id.slice(5)]();
			$.Timeline.create.invalidate();
		});

		$('#'+inp).off('blur').on('blur', function(){
			$.formElements.tipTxt.html("Back of the back, back of the back. Who on Barbie D? Everybody. Who you gotta see? Honestly, on my odyssey. I'm the baddest B, I don't even know how to speak");
			$.formElements.tipBox.fadeOut();
			val = $(this).val();
			...
			$.Timeline.create.ajax('validate', data).success($.Timeline.create.validateResponse);
			$.Timeline.create.invalidate();
		});
	}
	...
	$.formElements.input_color.on('focusin', function() {
		...
		$.Timeline.create.data.color = $(this).val();
	...
	$("#edit-submit").off('click').on("click", function(e){
		...
		$.Timeline.create.randomSongQuote();

		$('.ov-title').text("Captcha verification");
		$.formElements.loader.fadeIn();

		grecaptcha.execute($.Timeline.create.data.reCAPTCHA_site_key, {action: 'login'}).then(function(token) {
			$('.ov-title').text("Signup request");
			$.Timeline.create.ajax('create-user', {formId: $.formElements.formId.val(), color: $.Timeline.create.data.color, 'captcha-response': token}).success(function(r){
				if (!!r.response.error) {
					$.Timeline.create.error("Looks like we got lost on our way to Eden:", r.response.error);
				} else {
					var texts = ['YAY!', 'WooHoo!', 'Wahoo!', 'Wee!', 'Yee-Haw!', 'Yipeeeee!', 'YEAH!', 'Puffle-cious!', 'Snowy-PaRTYYY!'];
					// registered successfully!
					$.Timeline.create.error("Account registered successfully. Lastly, check your email and follow the instructions to activate your account.");
...
$.Timeline.create.randomSongQuote = function() {
...
$( function () {
	$.Timeline.create.elements();
	$.Timeline.create.setup();

	grecaptcha.ready(function() {
		grecaptcha.execute($.Timeline.create.data.reCAPTCHA_site_key, {action: 'homepage'}).then(function(token) {
			// initiate register
			$.Timeline.create.ajax('init', {'captcha-response': token}).success(function(r){
				...
				if (!!r.response.error) {
					...
					$.Timeline.create.error('Unable to authenticate captcha. Contact CPPS support:', r.response.error)
				}

				$.formElements.formId.val(r.response.formId);
				$.formElements.anonToken.val(CryptoJS.MD5($.Timeline.create.cookie.read("PHPSESSID")).toString());
				$.formElements.loader.fadeOut();
