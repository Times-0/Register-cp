...
$.Timeline.activate.cookie = {};
$.Timeline.activate.cookie.read = function (name) {
    var nameEQ = name + "=";
...
$.formElements = Timeline.activate.elements = function () {
	$.formElements = $.Timeline.activate.elements = {
...
Timeline.activate.ajax = function(request, data) {
	request = {
		url: $.Timeline.activate.data.base_url + '/ajax/?action=' + encodeURIComponent(request),
		type: 'post',
		data: data,
		error: function(e){
			...
			$.Timeline.activate.error("Unable to contact register server. Contact CPPS support.");
		},
		success: function(r) {
			if (!!!r) {
...
Timeline.activate.error = function (txt, errors) {
...
$.Timeline.activate.setup = function() {
	params={};location.search.replace(/[?&]+([^=&]+)=([^&]*)/gi,function(s,k,v){params[k]=decodeURIComponent(v)});
	if (!!!params['u'] || !!!params['k']) {
		$.Timeline.activate.error("Someone ate all the data", ['Unable to fetch username and activation code, please try again or contact support.']);
		$.formElements.loader.fadeOut();
	...
	$("#edit-activate").off('click').on("click", function(e){
		...
		grecaptcha.execute($.Timeline.activate.data.reCAPTCHA_site_key, {action: 'login'}).then(function(token) {
			$('.ov-title').text("Activating");
			$.Timeline.activate.ajax('activate', {formId: $.formElements.formId.val(), k: $("#edit-activationcode").val(), u: $("#edit-name").val(), 'captcha-response': token}).success(function(r){
				if (!!r.response.error) {
					$.Timeline.activate.error("Looks like we got lost on our way back from Utipoia:", r.response.error);
				} else {
					var texts = ['YAY!', 'WooHoo!', 'Wahoo!', 'Wee!', 'Yee-Haw!', 'Yipeeeee!', 'YEAH!', 'Puffle-cious!', 'Snowy-PaRTYYY!'];
					// registered successfully!
					$.Timeline.activate.error("Account activated successfully! You can now rock-a-party, whatcha waitin for?");
...
$.Timeline.activate.randomSongQuote = function() {
...
$( function () {
	$.Timeline.activate.elements();
	$.Timeline.activate.setup();

	grecaptcha.ready(function() {
		grecaptcha.execute($.Timeline.activate.data.reCAPTCHA_site_key, {action: 'homepage'}).then(function(token) {
			// initiate activation
			$.Timeline.activate.ajax('init', {'captcha-response': token}).success(function(r){
				...
				if (!!r.response.error) {
					...
					$.Timeline.activate.error('Unable to authenticate captcha. Contact CPPS support:', r.response.error)
				}
